...
export const createOrder = async (req, res) => {
    try{
        ...
        const ordered = await Order.create({order_makanan, total_price, status, CustomerId:CustomerId, TableId : TableId});
...
export const updateOrder = async (req, res) => {
    try{
        const { id } = req.params;
        const { order_makanan, total_price, status, TableId } = req.body;
        const [updated] = await Order.update({ order_makanan, total_price, status, TableId: TableId }, { where: { id } });
